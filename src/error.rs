...
use hyper::error::Error as HyperError; // Hyper error
...
#[derive(Debug)]
pub enum Error {
    /// General error type used by keepme core
    AccessDenied,
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Hyper(ref e) => fmt::Display::fmt(e, f),
...
impl StdError for Error {
    fn description(&self) -> &str {
        match *self {
            ref Perimeter => "Perimeter service error",
            ref AuthentificationError => "Authentification failed",
...
mod tests {
    ...
    fn test_from() {
        ...
        from_and_cause!(HyperError::Method => Hyper(..));
    }
